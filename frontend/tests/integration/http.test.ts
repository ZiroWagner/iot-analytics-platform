...
import { apiClient, getApiBaseUrl, getAuthUrl, isAuthenticated } from '@/shared/infrastructure/http/api-client'
import { API_BASE_URL, API_PREFIX, API_URL } from '@/shared/infrastructure/http/config'
...
describe('apiClient', () => {
  ...
  it('throws JSON stringified error for unexpected error format', async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({ code: 'ERR_001', info: { foo: 'bar' } }), {
        status: 500,
      }),
    )
    vi.stubGlobal('fetch', fetchMock)

    await expect(apiClient(API_ENDPOINTS.PROJECTS.LIST)).rejects.toThrow('ERR_001')
  })
})
