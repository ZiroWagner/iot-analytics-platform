...
describe('TelemetryStore', () => {
  ...
  it('should set initial state', () => {
    ...
    useTelemetryStore.getState().setInitialState('p1', snapshot as any)
  ...
  it('should apply a batch of events', () => {
    ...
    useTelemetryStore.getState().applyBatch(events as any)
    
    expect(useTelemetryStore.getState().devices['d1'].sensors['s1']).toBe(10)
  })

  it('should mark a device offline', () => {
    useTelemetryStore.setState({ devices: { 'd1': { status: 'online' } } as any })
  ...
  it('should clear devices', () => {
    useTelemetryStore.setState({ devices: { 'd1': {} } as any })
    useTelemetryStore.getState().clearDevices()
