    ...
    stages {
        ...
        stage('Security Scan (ZAP)') {
            steps {
                ...
                sh '/usr/local/bin/zap -cmd -quickurl http://frontend:3000 -quickout reports/zap-frontend-report.html || true'
                echo "Ejecutando escaneo de seguridad OWASP ZAP en Backend..."
                sh '/usr/local/bin/zap -cmd -quickurl http://backend:3001 -quickout reports/zap-backend-report.html || true'
            }
        }
        ...
        stage('Performance Scan (k6)') {
            steps {
                ...
                sh '/usr/local/bin/k6 run -e TARGET_URL=http://backend:3001 tests/performance/load-test.js'
            }
        }
    }
