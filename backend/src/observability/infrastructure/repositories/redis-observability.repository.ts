...
export class RedisObservabilityRepository implements ObservabilityRepositoryInterface {
    ...
    async getStreamLength(): Promise<number> {
        try {
            const xinfo = await (this.redisService.client as any).xinfo('STREAM', STREAM_NAME) as string[];
    ...
    async getConsumerLag(): Promise<number> {
        try {
            const groups = await (this.redisService.client as any).xinfo('GROUPS', STREAM_NAME) as string[][];
    ...
    async countOnlineDevicesForUser(userId: string): Promise<number> {
        try {
            ...
            const pipeline = this.redisService.client.pipeline();
            devices.forEach((d) => pipeline.hmget(`device:state:${d.id}`, 'status', 'lastSeenAt'));
            const results = await pipeline.exec();
            if (!results) return 0;
            ...
            for (const [, value] of results) {
                const [status, lastSeenAt] = (value as Array<string | null>) || [null, null];
    ...
    async scanActiveDeviceIds(): Promise<string[]> {
        ...
        let cursor = '0';
        do {
            const result = await this.redisService.client.scan(cursor, 'MATCH', 'device:state:*', 'COUNT', 100);
            cursor = result[0];
            const keys = result[1];
            if (keys.length > 0) {
    ...
    async getDeviceStates(deviceIds: string[]): Promise<Array<{
        deviceId: string;
        status: string | null;
        ...
        for (let i = 0; i < deviceIds.length; i++) {
            ...
            const [status, lastSeenAt] = (stateValues as Array<string | null>) || [null, null];
            states.push({
    ...
    async markDeviceOffline(deviceId: string): Promise<void> {
        const stateUpdate = ObservabilityDomainService.buildDeviceStateUpdate(deviceId);
        await this.redisService.client.hset(stateUpdate.key, stateUpdate.data);
    }

    async broadcastOfflineEvent(deviceId: string, projectId: string): Promise<void> {
        const event = ObservabilityDomainService.buildOfflineEvent(deviceId, projectId);
        await this.redisService.client.publish('telemetry:broadcast', JSON.stringify(event));
    }

    private async scanDeviceStates(): Promise<Array<{ status: string | null; lastSeenAt: string | null }>> {
        const states: Array<{ status: string | null; lastSeenAt: string | null }> = [];
        let cursor = '0';
        do {
            const result = await this.redisService.client.scan(cursor, 'MATCH', 'device:state:*', 'COUNT', 200);
            cursor = result[0];
            const keys = result[1];
