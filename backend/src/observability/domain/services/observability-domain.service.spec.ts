import { ObservabilityDomainService } from '../services/observability-domain.service';
...
describe('ObservabilityDomainService', () => {
  ...
  describe('calculateEventsPerSecond', () => {
    it('returns the rounded average of EPS values', () => {
      expect(ObservabilityDomainService.calculateEventsPerSecond([10, 20, 30])).toBe(20);
    });

    it('rounds down fractional averages', () => {
      expect(ObservabilityDomainService.calculateEventsPerSecond([1, 2])).toBe(2);
    });
  });
  ...
  describe('parseRedisStatus', () => {
    it('returns online for "online" string', () => {
      expect(ObservabilityDomainService.parseRedisStatus('online')).toBe('online');
    });
    ...
    it('returns offline for any other string', () => {
      expect(ObservabilityDomainService.parseRedisStatus('unknown')).toBe('offline');
    });
  });

  describe('extractLagFromGroup', () => {
    it('extracts lag value from group info array', () => {
      expect(ObservabilityDomainService.extractLagFromGroup(['name', 'mygroup', 'lag', '42'])).toBe(42);
    });

    it('returns 0 when lag key is not present', () => {
      expect(ObservabilityDomainService.extractLagFromGroup(['name', 'mygroup'])).toBe(0);
    });

    it('returns 0 when lag value is not a number', () => {
      expect(ObservabilityDomainService.extractLagFromGroup(['lag', 'invalid'])).toBe(0);
    });
  });

  describe('extractStreamLength', () => {
    it('extracts length from XINFO array', () => {
      expect(ObservabilityDomainService.extractStreamLength(['length', '500', 'groups', '1'])).toBe(500);
    });

    it('returns 0 when length key is not found', () => {
      expect(ObservabilityDomainService.extractStreamLength(['groups', '1'])).toBe(0);
    });
  });

  describe('buildDeviceStateUpdate', () => {
    it('returns the correct Redis key and offline data', () => {
      const result = ObservabilityDomainService.buildDeviceStateUpdate('dev123');
