...
export interface TelemetryAdapterInterface {
    ...
    buildInitialState(projectId: string): Promise<{ projectId: string; devices: Record<string, any> }>;
}
...
export class RedisTelemetryAdapter implements TelemetryAdapterInterface {
    ...
    async buildInitialState(projectId: string): Promise<{ projectId: string; devices: Record<string, any> }> {
        ...
        do {
            ...
            deviceIds.forEach((id: string) => projPipeline.get(`device:${id}:project`));
