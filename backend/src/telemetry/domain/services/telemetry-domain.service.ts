...
export class TelemetryDomainService {
    ...
    static groupEventsByProject(events: TelemetryEvent[]): Map<string, TelemetryEvent[]> {
  ...
  static buildTelemetryBatchWithStatus(
      ...
      projectId: string;
      devices: Record<string, { status: string; lastSeenAt: string; sensors: any[] }>;
      count: number;
      timestamp: string;
  } {
    ...
    static serializeSnapshotForRedis(snapshot: DeviceSnapshot): Record<string, string> {
        return {
