...
export class StreamIngestProcessor implements OnModuleInit, OnModuleDestroy {
    ...
    private async ensureConsumerGroup(): Promise<void> {
        try {
            await this.redisService.client.xgroup('CREATE', this.streamName, this.groupName, '$', 'MKSTREAM');
            this.logger.log(`Created consumer group ${this.groupName} on stream ${this.streamName}`);
        } catch (error: unknown) {
            ...
            try {
                ...
                if (result && result.length > 0) {
                    ...
                    if (messages.length > 0) {
                        this.logger.log(`Received batch of ${messages.length} payloads from stream`);
    ...
    private async processBatch(messages: any): Promise<void> {
        ...
        const { dataPoints, messageIds } = await this.prepareInsertData(parsedMessages);
        ...
        if (messageIds.length > 0) {
            await this.redisService.client.xack(this.streamName, this.groupName, ...messageIds);
        }
    }

    private async prepareInsertData(parsedMessages: ParsedStreamMessage[]): Promise<{
        ...
        const sensorMap = new Map(dbSensors.map((s) => [`${s.deviceId}:${s.name}`, s.id]));
        const { dataPoints, messageIds } = IngestDomainService.prepareDataPoints(parsedMessages, sensorMap);

        return { dataPoints, messageIds };
    }

    private async updateDeviceTimestamps(parsedMessages: ParsedStreamMessage[]): Promise<void> {
        const deviceTimestamps = IngestDomainService.getLatestTimestamps(parsedMessages);
        const updatePromises = Array.from(deviceTimestamps.entries()).map(([devId, ts]) =>
            this.prisma.device
                .update({
                    where: { id: devId },
                    data: { lastSeenAt: ts },
                })
                .catch((error) => this.logger.warn(`Failed to update lastSeenAt for ${devId}`, error)),
        );
