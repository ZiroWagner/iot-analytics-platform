...
export class DashboardsController {
  ...
  async getConfig(
    ...
    const config = await this.getDashboardConfigUseCase.execute(
      req.user.sub,
      projectId,
    );
    console.log(
  ...
  @ApiBody({
    schema: {
      properties: {
        layoutConfig: { type: 'object', example: { widgets: [] } },
      },
    },
  })
  ...
  async saveConfig(
    ...
    console.log(
      'DASHBOARD POST - projectId:',
      projectId,
      'body:',
      JSON.stringify(body, null, 2),
    );
    const config = await this.saveDashboardConfigUseCase.execute(
      req.user.sub,
      projectId,
      body.layoutConfig,
    );
    console.log(
      'DASHBOARD POST - saved config:',
      JSON.stringify(config, null, 2),
    );
    return {
      id: config.id,
